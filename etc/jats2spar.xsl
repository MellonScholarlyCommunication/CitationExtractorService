...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:template match="subj-group">
        <xsl:param name="s" tunnel="yes" />
        <xsl:variable name="subject-counting" select="count(ancestor-or-self::subj-group)" />
        ...
        <xsl:if test="subj-group">
            <xsl:call-template name="single">
                ...
                <xsl:with-param name="o" select="concat($s,'-term',$subject-counting+1)" tunnel="yes" />
            </xsl:call-template>
        </xsl:if>
    ...
    <xsl:template match="subject">
        ...
        <xsl:call-template name="double">
            ...
            <xsl:with-param name="p2" select="'rdfs:label'" tunnel="yes" />
            <xsl:with-param name="o2" select="." tunnel="yes" />
        </xsl:call-template>
    </xsl:template>
    ...
    <xsl:template match="@person-group-type[. = 'translators']">
        <xsl:param name="e" tunnel="yes" />
        <xsl:call-template name="assert">
            <xsl:with-param name="triples" select="(
                $e,'rdf:type','&foaf;Group',
